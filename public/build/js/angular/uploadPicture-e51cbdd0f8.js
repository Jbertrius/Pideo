...
app.controller('MyCtrl', ['$scope', 'Upload', '$timeout', '$sce',  function ($scope, Upload, $timeout, $sce ) {
    ...
    $scope.hint = true;
    $scope.formInfo = true;
    $scope.progress = 'Envoi des données en cours - 0 %'
    ...
    $scope.uploadFiles = function (files, audio) {
        ...
        if (files && files.length && audio && audio.length) {
            Upload.upload({
                ...
                if (ratio < 1)
                    $scope.progress = 'Envoi des données en cours - ' + Math.min(100, parseInt(100.0 * ratio)) + ' %';
                else 
                    $scope.progress = 'Génération en cours ...'
            });
        }
    };
